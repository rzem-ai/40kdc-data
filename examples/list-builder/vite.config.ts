 ...
 * touch `node:fs`/`node:url` at module-load time. The builder never calls them,
...
function stubNodeOnlyModules(): Plugin {
  ...
  return {
    name: "list-builder:stub-node-only-modules",
    enforce: "pre",
    resolveId(id) {
      // Real polyfill: ListForge URLs are gzipped — route node:zlib's
      // gunzipSync through fflate for browser builds.
      if (id === "node:zlib") return "\0lb-stub:node:zlib";
      if (stubbed.includes(id)) return "\0lb-stub:" + id;
...
export default defineConfig({
  ...
  resolve: {
    // The builder + datacard were lifted from the host app, which uses a `$lib`
    // alias; keep it so the copied files need no import rewrites.
    alias: {
